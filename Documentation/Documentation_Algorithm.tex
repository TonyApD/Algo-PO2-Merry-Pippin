...
Before the output is printed, the value is rounded. This is done, since all the groups between all dividers should be rounded which means the sum of the groups should also be a rounded value. The rounding is done by a util class where we stored all the util functions we need. This can be seen in ``Util.java''.
